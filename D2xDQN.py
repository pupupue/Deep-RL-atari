...
class D2xDQNagent(object):
    def __init__(self, state_shape, action_shape, params):
        ...
        """   Qnet   """
        """
        Uses dueling network
        + Value, Advantage is split
        + ./neurals/dueling_neurals.py 
        """
        self.DDQN = DDQN
        self.Qnet = DDQN(state_shape, action_shape, device).to(device)
        self.Qnet.apply(utils.weights_initializer.xavier)

        self.Qnet_optimizer = torch.optim.Adam(self.Qnet.parameters(), lr=self.learning_rate)

        """   Tnet   """
        if self.params['use_target_network']:
            self.Tnet = self.DDQN(state_shape, action_shape, device).to(device)
    ...
    def epsilon_greedy_Q(self, observation):
        ...
        self.step_num += 1
        if self.params["test"]:
            action = np.argmax(self.Qnet(observation).data.to(torch.device(device)).numpy())
        else:
            # else epsilon q
            if self.ep_num % 10 == 0:
                writer.add_scalar("DQL/epsilon", self.epsilon_decay(self.step_num), self.step_num)
            if random.random() < self.epsilon_decay(self.step_num):
                """   Problem here is once in 6 it also randomly chooses optimal strategy   """
    ...
    def learn_from_batch_experience(self, experiences, b_idx=None, is_weights=None):
        ...
        if not self.params['use_target_network']:
            T_target = reward_batch + gamma_arr * ~done_batch * self.Qnet(next_obs_batch).detach().max(1)[0].data.cpu().numpy()

        if self.params['use_target_network']:
            """copy over new weights to Tnet"""
            # A schedule can be used instead to vary the update freq.
            if self.ep_num % self.params['target_network_update_freq'] == 100: #so we wait for 100 ep not 1k to update Tnet
                self.Tnet.load_state_dict(self.Qnet.state_dict())
            T_target = reward_batch + gamma_arr * ~done_batch * self.Tnet(next_obs_batch).max(1)[0].data.cpu().numpy()
    ...
    def load(self, env_name):
        ...
        self.Qnet.load_state_dict(agent_state["Q"])
        self.Qnet.to(device)
    ...
    def get_Qvalues(self, obs):
        ...
        Q_val = self.Qnet(obs, test=True).data.to(torch.device(device)).numpy()
        return Q_val.tolist()
    """
    CALC STATS
...
if __name__ == "__main__":
    ...
    agent = D2xDQNagent(observation_shape, action_shape, agent_params)
    ...
    writer.close()
            ...
            # if global_step_num == 80000:
            #     snapshot1 = tracemalloc.take_snapshot()
            # if global_step_num == 120000:
            #     snapshot2 = tracemalloc.take_snapshot()
            # if global_step_num == 160000:
            #     snapshot3 = tracemalloc.take_snapshot()
            # if global_step_num == 200000:
            #     snapshot4 = tracemalloc.take_snapshot()
            # if global_step_num == 240000:
            ...
            #     top_stats3 = snapshot4.compare_to(snapshot3, 'lineno')
            #     top_stats4 = snapshot4.compare_to(snapshot1, 'lineno')
            #     print("----")
